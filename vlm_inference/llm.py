...
class LLM:
    ...
    def start_server(self):
        ...
        cmd = [
            ...
            "--max-model-len", str(self.max_input_tokens),
            "--max-num-seqs", str(1),                   # 限制同时处理的序列数
            "--max_num_batched_tokens", str(2048),      # chunked prefill 推荐值
            "--enable_chunked_prefill", str(True),       # 减少内存碎片
        ...
        if self.max_input_tokens is not None:
            cmd.extend(["--max-model-len", str(self.max_input_tokens)])
        
        # Start the server as a subprocess
        with open("serve2.log", "w") as log_file:
