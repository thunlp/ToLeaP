...
class ModifyAgenda(API):
    ...
    input_parameters = {
        ...
        'time': {'type': 'str', 'description': 'The new time for agenda. Format: %Y-%m-%d %H:%M:%S'},
        'location': {'type': 'str', 'description': 'The new location of the agenda.'},
    }
    output_parameters = {
    ...
    def call(self, token: str, content: str, time: str, location: str) -> dict:
        ...
        try:
            status = self.modify_agenda(token, content, time, location)
    ...
    def modify_agenda(self, token: str, content: str, time: str, location: str) -> str:
